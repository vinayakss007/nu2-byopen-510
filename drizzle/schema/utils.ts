...
const getTableName = (column: any) => (column as any).table?.[Symbol.for('drizzle:Name')] || 'table';

export const tenantIdx = (table: any) => index(`idx_${getTableName(table.tenantId)}_tenant`).on(table.tenantId);
export const metadataIdx = (table: any) => index(`idx_${getTableName(table.metadata)}_metadata_g`).using('gin', table.metadata);
export const activeIdx = (table: any) => index(`idx_${getTableName(table.id)}_active`).on(table.id).where(sql`deleted_at IS NULL`);

...
export interface TableRegistryEntry<T extends PgTableWithColumns<AnyPgColumn> = PgTableWithColumns<AnyPgColumn>> {
  table: T;
...
export const TABLE_REGISTRY = {
  ...
  segmentMembers: {
    ...
    metadata: {
      ...
      hasAudit: false,
      hasMetadata: false,
      dependencies: ['segments', 'tenants'],
      description: 'Segment membership cache',
      isCore: false,
      indexes: ['idx_segment_members_pk', 'idx_segment_members_tenant'],
    },
  },
} satisfies Record<string, TableRegistryEntry>;
...
/**
 * Simplified access to all tables.
 * Use this for type-safe table access without metadata.
 */
export const TABLES = Object.fromEntries(
  Object.entries(TABLE_REGISTRY).map(([name, entry]) => [name, entry.table])
) as {
  [K in keyof typeof TABLE_REGISTRY]: (typeof TABLE_REGISTRY)[K]['table'];
};
 ...
 */
export function getTablesByGroup(group: SchemaGroup): Record<string, PgTableWithColumns<AnyPgColumn>> {

...
export function encryptSensitiveFields<T extends Record<string, any>>(
  ...
  for (const [key, value] of Object.entries(result)) {
    if (fieldsToEncrypt.some(f => key.toLowerCase().includes(f.toLowerCase())) && value) {
      result[key as keyof T] = encrypt(String(value)) as any;
    }
  }
...
export function decryptSensitiveFields<T extends Record<string, any>>(
  ...
  for (const [key, value] of Object.entries(result)) {
    if (fieldsToDecrypt.some(f => key.toLowerCase().includes(f.toLowerCase())) && value) {
      try {
        result[key as keyof T] = decrypt(String(value)) as any;
      } catch {

  ...
  try {
    ...
    for (const stmt of statements) {
      if (!stmt.toUpperCase().includes('DROP') && !stmt.toUpperCase().includes('DELETE FROM')) {
        ...
        // Only allow safe DROP statements
        if (stmt.toUpperCase().includes('DROP TABLE IF EXISTS') ||
            ...
            stmt.toUpperCase().includes('DROP VIEW IF EXISTS')) {
          console.log(`🔄 Executing safe DROP: ${stmt.substring(0, 100)}...`);
          await client.query(stmt + ';');
        } else {
          // Skip potentially dangerous statements
          console.log(`🚫 Skipping statement: ${stmt.substring(0, 100)}...`);
        }
      }
    }

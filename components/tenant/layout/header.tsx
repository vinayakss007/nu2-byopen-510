            ...
            <div className="absolute right-0 top-full mt-1.5 w-80 bg-card border border-border rounded-xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-4 py-3 border-b border-border flex items-center justify-between bg-muted/20">
                ...
                {unread > 0 && <button onClick={async () => {
                  await fetch('/api/tenant/notifications/mark-all-read', { method: 'POST' });
                ...
                {notifications.length === 0 ? (
                  ...
                  <div className="divide-y divide-border/50">
                    {notifications.slice(0, 5).map((n: any) => {
                      ...
                      return (
                        <div key={n.id} onClick={() => { if(!n.is_read) markNotifRead(n.id); if(n.link) router.push(n.link); setShowNotifPanel(false); }}
                          className={cn("px-4 py-3 cursor-pointer hover:bg-accent transition-colors flex gap-3 items-start text-left", !n.is_read && "bg-violet-50/30 dark:bg-violet-900/5")}>
                          <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5", 
                            n.priority === 'high' ? "bg-red-100 dark:bg-red-900/30 text-red-600" : "bg-violet-100 dark:bg-violet-900/30 text-violet-600")}>
                            <Icon className="w-4 h-4"/>
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={cn("text-xs leading-normal", !n.is_read ? "font-bold text-foreground" : "font-medium text-muted-foreground")}>{n.title || n.message}</p>
                            {n.content && <p className="text-[10px] text-muted-foreground mt-0.5 line-clamp-2">{n.content}</p>}
                          ...
                          {!n.is_read && <div className="w-1.5 h-1.5 rounded-full bg-violet-600 mt-1.5 shrink-0 shadow-[0_0_8px_rgba(124,58,237,0.5)]"/>}
                        </div>
                      );
                    })}
                  </div>

...
export async function GET(request: NextRequest) {
  try {
    ...
    if (activeOnly) {
      query = query.where(and(eq(services.tenantId, tenantId), eq(services.isActive, true)));
    }
    
    if (category) {
      query = query.where(and(eq(services.tenantId, tenantId), eq(services.category, category)));
    }

  ...
  try {
    ...
    const data = await db
      .select({
        ...
        delivered_count: sql<number>`(SELECT count(*)::int FROM webhook_queue WHERE webhook_id = ${integrations.id} AND status = 'delivered')`,
        failed_count: sql<number>`(SELECT count(*)::int FROM webhook_queue WHERE webhook_id = ${integrations.id} AND status = 'failed')`
      })
      .from(integrations)
      .where(and(eq(integrations.tenantId, ctx.tenantId), eq(integrations.type, 'webhook')))

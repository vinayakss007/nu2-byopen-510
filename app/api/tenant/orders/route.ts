...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status) {
      query = query.where(and(eq(orders.tenantId, tenantId), eq(orders.status, status)));
    }

    if (contactId) {
      query = query.where(and(eq(orders.tenantId, tenantId), eq(orders.contactId, contactId as any)));
    }

    const offset = (page - 1) * limit;
    const results = await query.orderBy(desc(orders.createdAt)).limit(limit).offset(offset);

    const [{ total }] = await db.select({ count: count() }).from(orders).where(eq(orders.tenantId, tenantId));

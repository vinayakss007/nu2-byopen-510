...
import { eq, and, sql, desc, asc, ne, isNull } from 'drizzle-orm';
  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const [role] = await tx.select()
        .from(roles)
        .where(and(eq(roles.tenantId, ctx.tenantId), eq(roles.slug, role_slug)))
        .limit(1);
  ...
  try {
    ...
    if (action === 'change_role') {
      ...
      const [role] = await db.select()
        .from(roles)
        .where(and(eq(roles.tenantId, ctx.tenantId), eq(roles.slug, roleSlug)))
        .limit(1);
      ...
      await logAudit({ tenantId: ctx.tenantId, userId: ctx.userId, action: 'role_change', resourceType: 'member', resourceId: target.userId, newData: { role: roleSlug } });
      ...
      await logAudit({ tenantId: ctx.tenantId, userId: ctx.userId, action: 'member_removed', resourceType: 'member', resourceId: target.userId, newData: { reassigned_to: reassignTo, contacts: cCount?.n, deals: dCount?.n } });

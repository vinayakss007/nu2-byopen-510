...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status) {
      query = query.where(and(eq(invoices.tenantId, tenantId), eq(invoices.status, status)));
    }

    if (contactId) {
      query = query.where(and(eq(invoices.tenantId, tenantId), eq(invoices.contactId, contactId as any)));
    }

    const offset = (page - 1) * limit;
    const results = await query.orderBy(desc(invoices.createdAt)).limit(limit).offset(offset);

    const [{ total }] = await db.select({ count: count() }).from(invoices).where(eq(invoices.tenantId, tenantId));
...
export async function POST(request: NextRequest) {
  try {
    ...
    const invoiceNumber = `INV-${String(countResult[0].count + 1).padStart(5, '0')}`;

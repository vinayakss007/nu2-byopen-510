  ...
  try {
    ...
    const files = await db
      .select({
        ...
        original_name: fileAttachments.fileName, // Using fileName for both as per schema
        mime_type: fileAttachments.mimeType,
        size_bytes: fileAttachments.fileSize,
        storage_type: fileAttachments.storageType,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (tenantWithPlan && (tenantWithPlan.maxStorageGb ?? 0) > 0 && usedGb >= (tenantWithPlan.maxStorageGb ?? 0)) {
      return NextResponse.json({ error: `Storage limit (${tenantWithPlan.maxStorageGb} GB) reached. Upgrade your plan.` }, { status: 403 });
    }

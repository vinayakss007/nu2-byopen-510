...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tenantRow?.plan && tenantRow.plan.maxUsers! > 0 && (tenantRow.currentUsers ?? 0) >= tenantRow.plan.maxUsers!) {
      return NextResponse.json({ error: `User limit (${tenantRow.plan.maxUsers}) reached. Upgrade your plan.` }, { status: 403 });
    }

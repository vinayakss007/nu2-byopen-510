  ...
  try {
    ...
    const [tenantUsage, growth] = await Promise.all([
      db.select({
        ...
        contactPct: sql<number>`CASE WHEN ${plans.maxContacts} > 0 THEN round((${tenants.currentContacts}::numeric / ${plans.maxContacts} * 100))::int ELSE 0 END`,
        userPct: sql<number>`CASE WHEN ${plans.maxUsers} > 0 THEN round((${tenants.currentUsers}::numeric / ${plans.maxUsers} * 100))::int ELSE 0 END`,
      })
      .from(tenants)
      ...
      .from(usageSnapshots)
      .where(sql`${usageSnapshots.snapshotDate} > (CURRENT_DATE - 30)::text`)

  ...
  try {
    ...
    const [restoreLog] = await db
      ...
      .values({
        ...
        action: 'execute',
        status: 'pending',
        // In schema, selectiveRestoreLogs has: backupId, tenantId, action, status, startedAt, completedAt, errorMessage.
        // It doesn't have restore_mode, tables_selected, performed_by.
        // I will use metadata or add to errorMessage if critical.
      })
    ...
    await createAuditLog({
      ...
      performed_by_email: ctx.userEmail,
